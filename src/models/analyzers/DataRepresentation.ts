...
export const LOOKS_OPCODE_PREFIX = 'looks_';
...
class DataRepresentation extends Analyzer {
  ...
  private firstPointMotion(): void {
    // one point when at least a motion or a looks block is used
    for (const target of this.targets) {
      for (const _blockKey of Object.keys(target.blocks._blocks)) {
        const _block = target.blocks._blocks[_blockKey];
        if (_block.opcode.includes(MOTION_OPCODE_PREFIX) || _block.opcode.includes(LOOKS_OPCODE_PREFIX)) {
          this.score = 1;
          break;
        }
      }
